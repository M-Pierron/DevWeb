...
function UserProfile() {
  ...
  // Charger les données utilisateur depuis le contexte d'authentification
  useEffect(() => {
    if (user) {
      // Initialiser les données du profil avec les informations de l'utilisateur connecté
      setUserData(prevData => ({
        ...
        public: {
          ...prevData.public,
          pseudonyme: user.email ? user.email.split('@')[0] : '' // Utiliser la partie locale de l'email comme pseudonyme par défaut
        }
      }));
    }
  }, [user]);
  ...
  const handleSave = () => {
    // Mise à jour du profil utilisateur dans le système d'authentification
    // Dans une application réelle, vous feriez un appel API ici
    console.log('Données sauvegardées', userData);
    
    // Sauvegarder les données dans localStorage pour simuler la persistance
    const currentUser = JSON.parse(localStorage.getItem('user') || '{}');
    const updatedUser = {
      ...currentUser,
      firstName: userData.private.prenom,
      lastName: userData.private.nom,
      profile: userData.public
    };
  ...
  return (
    <>
        {editing && (
            <Nav name={"MODIFICATION DE PROFIL"} />
        )}
        {!editing && (
            <Nav name={"PROFIL"} />
        )}
